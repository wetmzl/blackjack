...
Trophy Room 是本地历史对局查看器：从主菜单进入，按新到旧显示结果。博士胜利的记录使用该角色独立的横版平躺败北图；失败或离席记录暂时使用透明图像占位。点击记录必须打开 Modal，显示最终双方左轮子弹数/容量、双方爆牌次数和双方黑杰克次数，不能使用卡片内展开。博士胜利且角色配置了 `trophyGallery` 时，详情 Modal 还要显示 5:7 的档案式昏迷头肩头像，并提供 2:3 竖屏全屏鉴赏；鉴赏底图可以配置任意数量的方形局部特写点，每个点包含唯一 ID、名称、百分比坐标、图片和描述。头像旁的固定文案使用角色目录 Tier 动态显示“战利品等级：{tier}”，标题动态显示“{角色名}的死体展示”，不得在角色 JSON 里重复维护这两段值。失败或离席记录不得显示这些战利品鉴赏资源。
