# 项目总提示词：Android 优先的日式二次元 PWA「策展人的魔鬼牌局」
...
玩家扮演来自高位面的“策展人”。策展人向整个泰拉发出邀请，把回应者召入一座与外界隔绝的古堡。每名“与会者”都怀着足以令自己押上性命的执念而来，并在入席前换上策展人指定的服装。牌桌上的活动是策展人主持的魔鬼游戏——“黑杰克轮盘赌”。

公开规则是：每轮黑杰克的败者接受累积式轮盘惩罚。与会者受罚时，由发牌员把左轮对准其头部并扣动扳机；实弹击发会令与会者当场殒命。策展人受罚时枪口始终朝向天花板，因此不会死。若与会者最终获胜，策展人承诺实现其任意一个愿望，作为对方赌上性命的回报。

隐藏真相是：策展人即使落败，也能把时间回溯到本局开始以前，重新进行游戏，直到与会者死亡。与会者不可能知道经历过的失败时间线。角色档案可以交代与会者的执念和参局理由，但与会者的台词、认知与任何面向角色的叙事都不得泄露时间回溯真相。

叙事术语与现有领域标识固定映射如下；本轮只迁移说明和注释，不要求同步改名代码标识、具体台词或前端文案：

* `player` / `PLAYER`：策展人。
* `opponent` / `AI`：与会者。
* `staff`：发牌员；其共享左轮图层只负责瞄准与会者。
* `player-killed`：保留的兼容性领域标识，叙事上不是策展人死亡，而是枪口朝天击发、与会者按公开规则获胜；隐藏时间线随后回溯。
* `opponent-killed`：发牌员击发实弹，与会者当场死亡，策展人赢得最终保留的时间线。
* `unconscious`、`defeated`、`trophy` 等既有资源和代码名暂不重命名；在新剧情中，与会者被实弹命中后的同类资源一律按“死亡、遗体、收藏记录”解释，不再按昏迷演出解释。

后续大厅、说明、角色档案、对白和结算文案都必须以此契约为准；旧“博士 / 熟人牌局 / 假面舞会余兴”设定不得继续扩写。
...
Trophy Room 是策展人的本地对局与藏品记录查看器：从主菜单进入，按新到旧显示结果。策展人获胜、与会者死亡的记录使用该角色独立的横版遗体陈列图；策展人落败或中止牌局的记录暂时使用透明图像占位。点击记录必须打开 Modal，显示最终双方左轮子弹数/容量、双方爆牌次数和双方黑杰克次数，不能使用卡片内展开。策展人获胜且与会者配置了 `trophyGallery` 时，详情 Modal 还要显示 5:7 的档案式遗体头肩像，并提供 2:3 竖屏全屏收藏记录；底图可以配置任意数量的方形局部记录点，每个点包含唯一 ID、名称、百分比坐标、图片和描述。固定标题与等级文案继续从角色目录 Tier 和角色名动态生成，不得在角色 JSON 里重复维护。策展人落败或中止牌局的记录不得显示这些收藏资源。
...
未来用于完整表现发牌员瞄准与会者并执行致命枪击。策展人受罚时只表现枪口朝向天花板，不进入与会者的行刑构图。
...
interface CharacterDefinition {
  ...
  // 校准共享发牌员 7mm 左轮图层，使枪口对准该与会者受刑立绘的头部
  revolverPlacement: {
    top: number;
    left: number;
    mobileTop: number;
...
与会者立绘至少包括：轻松、纠结、嘲笑、被发牌员从画面外以 7mm 左轮抵住头部时的紧张状态，以及实弹击发后双眼失焦、上身失去支撑的牌桌死亡状态。既有资源槽仍名为 `unconscious`，但这只是兼容性名称，不能再据此把枪击结果描述成昏迷。除一两张具有明确特殊用途的结算或陈列图外，常规牌桌立绘必须以横向半身坐姿为主：身体重心落在座位后，手臂自然靠近桌沿，但透明资源中不画出椅子或桌子。牌桌死亡立绘同样不得增加椅子、桌子或其他状态立绘中没有的家具。每名与会者还必须拥有一张独立的策展人胜利总结立绘：遗体全身无力地留在椅子上，人物和椅子均完整可见；这张图只能用于与会者死亡后的 Match Summary，不得替代牌桌状态立绘。另需一张 16:4.5 的超宽陈列图供 Trophy Room 的策展人胜利记录使用，与会者遗体应全身仰躺在窄棺木式工业收纳舱中。扳机状态使用独立、可复用的“发牌员手 + 深色钢制 7mm 左轮”透明图层；发牌员使用中性灰白制服，枪口从画面右侧朝向当前受罚与会者的头部，与会者本人不得持枪。

对白需符合 W 轻佻、危险、爱看热闹的气质，但不得直接复制原作台词。

艾丽妮固定美术锚点：淡紫/粉紫眼，短齐刘海与极长银白高马尾；两侧黑色尖羽冠/角状饰件，中间白色褶饰；白高领、黑色四角星领扣、白褶边胸襟、黑泡袖/紧身长袖、宽白腕褶、黑手套、黑腰封及不对称腰甲、玫红系带与后腰飘带；象牙白长裙、黑褶边与黑缝线、白袜、黑色圆头搭带鞋。禁止短波波头、金眼、金色胸带、黑色低领改衣。常规态仍遵守 W 1536×1024 横向坐姿金样本。
...
领域层对双方继续复用同一套填弹与命中概率。表现与结局必须遵守叙事非对称性：`opponent` 命中代表发牌员射杀与会者；`player` 命中只代表策展人朝天花板击发并触发其落败分支，绝不表现为策展人中枪或死亡。
...
如果 Roulette 判定任一方命中，Match 结束。底层历史原因保留既有枚举名，但叙事语义不同：
...
* `player-killed`：策展人朝天花板的一枪击发；策展人不死，与会者在公开规则下赢得一次许愿资格，隐藏时间线随后被策展人回溯。
* `opponent-killed`：发牌员对与会者头部击发实弹，与会者当场死亡。
* `escaped`：策展人主动中止当前牌局。
...
* 纠结：思考手牌或高压状态
* 嘲笑：玩家失误或 W 赢下一轮
* 被瞄准：发牌员从画面右侧将 7mm 左轮抵近与会者头部，角色双手保持低位并显得紧张；德克萨斯的紧张表现可以更克制
* 死亡：实弹击发后双眼失焦、上身微微后仰，不增加椅子、桌子或其他家具；资源槽仍沿用 `unconscious`

人物身份、指定服装、比例、线条和光照必须在所有状态间保持一致。除结算和陈列等少量特殊图外，常规状态统一使用横向半身坐姿，并严格以 W 的 1536×1024、scale 1 构图为牌桌基准；其他与会者不得增加可变 portraitScale 入口。立绘应优先使用透明背景；扳机演出使用“被瞄准的紧张人物立绘 + 独立发牌员手持深色钢制 7mm 左轮”的 DOM 分层组合，发牌员图层应为中性灰白制服并可跨角色复用；每名与会者必须通过 `CharacterDefinition.revolverPlacement` 的 `top`、`left`、`mobileTop`、`mobileLeft` 分别校准桌面与窄屏道具，使枪口准确贴合该角色 threatened 立绘的头部。`.trigger-prop` 必须位于角色立绘之上（明确更高 z-index，避免被角色缩放堆叠上下文遮挡），但不得遮挡对话可读性；合成后的枪口只能朝向当前受罚与会者，不能朝策展人或第三者。策展人受罚的朝天花板枪击属于另一套表现，不复用与会者头部瞄准图层。
