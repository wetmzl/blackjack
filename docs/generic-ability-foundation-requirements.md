...
规则引擎必须保证下一次 Hit 不会进入无合法牌崩溃状态。兼容 fallback 需要根据 Hit 时的最新手牌重新计算合法牌；若因为异常旧存档或未来基础规则导致任何单张牌都无法形成 21，则保留原始待抽牌、消费武装状态并记录能力解析失败事件，不能调用 `nextInt(0)`。这是本次重构允许修复的现有崩溃缺陷，不属于技能平衡调整。
...
现有 `USE_SKILL` 可以在迁移期作为兼容输入，进入 reducer 后立即转换为 `PLAY_ABILITY`，但新历史记录和新测试应使用通用名称。
...
旧 `SKILL_USED`、`SKILL_ADVICE` 和 `TRIGGER_AVOIDED_BY_SKILL` 可在存档迁移周期保留读取兼容，但新对局应逐步改用通用事件。
...
本次重构会改变活动对局存档结构，应提升 Schema 版本并提供迁移：

- `rhodesArmed=true` 迁移为对应状态。
- `nightQueenArmed=true` 迁移为对应状态。
- 现有技能手牌 ID 保持不变。
- 无法找到定义 ID 时明确报错，不静默删除能力。
- 存档中的能力目录版本与当前版本不一致时必须执行显式迁移；没有迁移路径时拒绝恢复活动对局，避免定义变化破坏确定性。
...
- 迁移前 `rhodesArmed`、`nightQueenArmed` 存档可恢复为新状态。
