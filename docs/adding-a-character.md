...
6. 先验收“共享陈列舱 + 默认 `fullBody`”合成画面，再量记录点坐标。`x`、`y` 均是相对完整 2:3 合成画布左上角的 0–100 百分比；记录点只在默认姿势显示。同一部位有多个角度时使用不同 ID 和略微错开的记录点，例如 `feet-overhead` 与 `feet-side`。
7. 每个局部记录对象写入唯一 `id`、面向策展人的 `name`、`x`、`y`、方形 `image` 和与会者专属 `description`。数组没有上限，也不要求不同与会者拥有相同的记录点。
8. UI 固定文案不写入角色 JSON：收藏卡片直接以目录元数据渲染“`<tier>`级战利品”、角色名和首次击败日期；B/A/S/SS 分别使用蓝/紫/金/红色标签与边框。
