...
function toSkillDefinition(definition: AbilityDefinition): SkillDefinition {
  ...
  const timing: readonly SkillTiming[] = definition.activation.type === "action"
    ...
    : [];
  return Object.freeze({ id: definition.id, name: definition.name, category, description: definition.description, usage: definition.usage ?? definition.description, timing, unlock: definition.unlock });
}

export const SKILL_DEFINITIONS: readonly SkillDefinition[] = Object.freeze(ABILITY_DEFINITIONS.filter((definition) => definition.sourceKind === "player-skill").map(toSkillDefinition));
