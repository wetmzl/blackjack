...
export const MatchStateSchema = z.object({
  ...
  skills: z.object({ cards: z.array(z.string().min(1)) }).strict(), peekedCards: z.array(CardSchema), round: RoundSchema,
...
export const SaveFileV1Schema = z.object({
  format: z.literal(SAVE_FORMAT), schemaVersion: z.literal(1), gameVersion: z.string().min(1),
...
export const SaveFileV2Schema = z.object({
  format: z.literal(SAVE_FORMAT), schemaVersion: z.literal(2), gameVersion: z.string().min(1),
  createdAt: z.string().datetime({ offset: true }), updatedAt: z.string().datetime({ offset: true }),
  profile: PlayerProfileSchema, activeMatch: MatchStateSchema.nullable(), settings: GameSettingsSchema,
  history: z.array(MatchHistoryRecordSchema)
}).strict();
export const CurrentSaveFileSchema = SaveFileV2Schema;
export type SaveFileV1 = z.infer<typeof SaveFileV1Schema>;
export type SaveFileV2 = z.infer<typeof SaveFileV2Schema>;
/** Runtime schema uses mutable arrays; the domain-facing save type preserves MatchState immutability. */
export type SaveFile = Omit<SaveFileV2, "activeMatch" | "history"> & { activeMatch: MatchState | null; history: MatchHistoryRecord[] };
...
export function migrateV1ToCurrent(save: SaveFileV1): SaveFile {
  return { ...save, activeMatch: save.activeMatch as MatchState | null, schemaVersion: CURRENT_SCHEMA_VERSION, history: [] };
}

export function migrateV2ToCurrent(save: SaveFileV2): SaveFile {
  return { ...save, activeMatch: save.activeMatch as MatchState | null, history: [...save.history] };
}
...
export function validateAndMigrateSave(input: unknown): SaveFile {
  ...
  if (envelope.data.schemaVersion < 1) throw new SaveValidationError(`Unsupported save schema version: ${envelope.data.schemaVersion}`);
  if (envelope.data.schemaVersion === 1) {
    ...
    return migrateV1ToCurrent(parsed.data);
  }
  const parsed = SaveFileV2Schema.safeParse(input);
  if (!parsed.success) throw new SaveValidationError(`Invalid save data: ${parsed.error.issues[0]?.path.join(".") || "root"} ${parsed.error.issues[0]?.message ?? "unknown error"}`);
  return migrateV2ToCurrent(parsed.data);
}
