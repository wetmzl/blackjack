...
export function abilityTriggerNotice(events: readonly GameEvent[], opponentName: string): string | null {
  ...
  for (const event of events) {
    ...
    if (!definition || definition.rules.find((rule) => rule.id === event.ruleId)?.notify === false) continue;
    const key = `${event.owner}:${event.definitionId}`;
    if (seen.has(key)) continue;
    seen.add(key);
    const actor = event.owner === "player" ? "策展人" : opponentName;
    const effect = definition.rules.find((rule) => rule.id === event.ruleId)?.triggerNotice ?? definition.triggerNotice ?? definition.description;
